...
char *create_char_arr(size_t size) {
	char* char_arr=NULL;
	return char_arr = (char *)malloc(sizeof(char)*size);
}
...
char* read_input(FILE *stream,size_t *counter,bool *filled) {
	char *arr=NULL;
	char buffer[8096];
	if (stream==NULL){
		fprintf(stderr, "Error: Invalid file stream.\n");
        return NULL;
	}
	if(*counter==0) {
		if (fgets(buffer,sizeof(buffer),stream)==NULL || feof(stream)) {
			printf("first call, nothing in stream\n");
			return NULL;
		} else {
			printf("first call, something in stream\n");
			arr = create_string(8097);
			strncpy(arr,buffer,8096);
			(*counter)++;
			int clear;
	...
	if(fgets(buffer,sizeof(buffer),stream)!=NULL) {
		printf("Not first call, something in stream\n");
		size_t len = strlen(buffer);
		if(len>0 && (buffer[len-1]== '\n' || buffer[len-1] == '\0')) {
		arr = create_string(len+1);
		strcpy(arr,buffer);
		} else {
		printf("empty or incomplete striing\n");	
		}
		buffer[0]='\0';
	}
	printf("Not first call, nothing in stream\n");
	fflush(stdout);
	(*counter)++;
	return arr;
}

bool bool_parse_string(){
	return true;
}
